...
/// A FrameAllocator that returns usable frame from the bootloader's memory map.
pub struct BootInfoFrameAllocator {
    memory_map: &'static MemoryMap,
    // increased for every frame allocation.
    next: usize,
}

impl BootInfoFrameAllocator {
    // Create a FrameAllocator from the given memory map.
    // caller must gurantee that memory map is valid.
    ...
    fn usable_frames(&self) -> impl Iterator<Item = PhysFrame> {
        ...
        // transform to frame start addresses
...
pub fn create_example_mapping(
    ...
    // caller must ensure frame is not already in use
